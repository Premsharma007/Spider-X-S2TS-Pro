...
/* Make buttons rounded, slightly larger */
button {
  border-radius: 12px !important;
  padding: 10px 14px !important;
}

/* File + audio inputs rounded corners */
...
with gr.Blocks(theme=dark, css=CUSTOM_CSS, fill_height=True) as demo:
    ...
    <div class="fade-in" style="display:flex;align-items:center;gap:14px;margin: 6px 2px 16px 2px;">
      <div style="width:42px;height:42px;border-radius:12px;background:linear-gradient(135deg,#7c3aed55,#06b6d455);display:flex;align-items:center;justify-content:center;border:1px solid rgba(124,58,237,0.35);">
        <span style="font-size:22px;">🕷️</span>
      </div>
      <div>
        <div style="font-size:20px;font-weight:700;">Spider『X』 Full Speech Pipeline</div>
        <div style="opacity:0.8">ASR (Whisper) → MT (IndicTrans2) → TTS (IndicF5), GPU-safe & elegant</div>
      </div>
    </div>
    """)
    ...
    with gr.Row():
        with gr.Column(scale=1):
            with gr.Group():
                ...
                gr.Markdown("<div class='footer-note'>Note: TTS requires a reference voice audio + its transcript for best results.</div>")
